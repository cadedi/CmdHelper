...
namespace LinuxCmdHelper.Services
{
    public class CommandRepository
    {
        ...
        {
            // 确保 Windows Server 2016 兼容 TLS 1.2
            try
            {
            ...
            HttpClientInstance.DefaultRequestHeaders.Add("User-Agent", "LinuxCmdHelper/2.0 (Windows NT; Portable)");
        }

        private readonly List<CommandItem> _commands = new List<CommandItem>();
        private readonly List<DataSourceConfig> _dataSources = new List<DataSourceConfig>();
        ...
        public List<CommandItem> Search(string category, string query)
        {
            ...
            if (!string.IsNullOrEmpty(category) && category != "全部场景")
            {
        ...
        public void MergeCommands(List<CommandItem> incoming, string mergeMode, out int newAdded, out int updated)
        {
            ...
            // merge 模式：按 id 建立映射，同 id 覆盖更新，新 id 追加
        ...
        public List<CommandItem> ParseCommandsFromJson(string json)
        {
            ...
            try
            {
                // 2. 尝试解析带有 code/data/list/commands 等包装格式的 JSON 对象
        ...
        public void UpdateDataSource(DataSourceConfig ds)
        {
            int idx = _dataSources.FindIndex(d => d.Id == ds.Id);
            if (idx >= 0)
            {
                _dataSources[idx] = ds;
                SaveDataSourcesConfig();
        ...
        public bool ExportToSingleHtml(string targetFilePath, out string error)
        {
            ...
            try
            {
                ...
                {
                    string webFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "LinuxCmdHelper_Web.html");
